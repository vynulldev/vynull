...
// loop_time 0: it must parse as unset (colour_id 0, which renders green via the
// palette) and NOT be treated as a loop.
func TestParseANLZCuesUnsetNotLoop(t *testing.T) {
	...
	if cues[0].ColorID != 0 {
		t.Errorf("ColorID = %#x, want 0 (unset; green via palette)", cues[0].ColorID)
	}
	if cues[0].IsLoop {
