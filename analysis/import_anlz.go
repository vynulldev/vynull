...
func ParseANLZ(datPath, extPath, twoEXPath string, bpm float64, durationSec int) *Result {
	var twoEX []byte
	if twoEXPath != "" {
		twoEX = readFileOrNil(twoEXPath)
	}
	return ParseANLZBytes(readFileOrNil(datPath), readFileOrNil(extPath), twoEX, bpm, durationSec)
}

// ParseANLZBytes is ParseANLZ over the raw bytes of the .DAT/.EXT/.2EX files,
// for callers that fetch ANLZ over the network (e.g. an external player's media
// over NFS) rather than reading from disk. Any argument may be nil.
//
...
func ParseANLZBytes(dat, ext, twoEX []byte, bpm float64, durationSec int) *Result {
	r := &Result{
	...
	if body := anlzSectionBodyData(dat, tagPWAV); body != nil {
		r.WavePreviewANLZ = body
		got = true
	}
	if body := anlzSectionBodyData(dat, tagPWV2); body != nil {
		r.WaveTinyANLZ = body
		got = true
	}
	if body := anlzSectionBodyData(dat, tagPQTZ); body != nil {
	...
	if body := anlzSectionBodyData(ext, tagPWV3); body != nil {
		r.WaveDetailMono = body
		got = true
	}
	if body := anlzSectionBodyData(ext, tagPWV4); body != nil {
		r.WaveColorPreview = body
		got = true
	}
	if body := anlzSectionBodyData(ext, tagPWV5); body != nil {
		r.WaveDetail = body
		got = true
	}
	...
	if blob := readANLZSection(ext, tagPQT2); blob != nil {
		r.BeatGridPQT2 = blob
		got = true
	}
	...
	if pssi := anlzSectionAfterHeaderData(ext, tagPSSI); pssi != nil {
	...
	if body := anlzSectionBodyData(twoEX, tagPWV6); body != nil {
		r.WavePreview3Band = body
		got = true
	}
	if body := anlzSectionBodyData(twoEX, tagPWV7); body != nil {
		r.WaveDetail3Band = body
		got = true
	}
	if body := anlzSectionBodyData(twoEX, tagPWVC); body != nil {
...
// readFileOrNil reads a file, returning nil on any error (a missing .EXT/.2EX is
// normal), so section lookups just find nothing.
...
// anlzSectionBodyData returns the raw entry data of a section (the bytes at
// [len_header:len_tag]) from in-memory ANLZ file bytes, or nil if absent.
func anlzSectionBodyData(data []byte, tag string) []byte {
	blob := readANLZSection(data, tag)
	if blob == nil {
		return nil
	}
...
// anlzSectionAfterHeaderData returns the section bytes after the generic 12-byte
// header (extra + body together) from in-memory ANLZ file bytes. Used for PSSI,
// whose downstream consumer expects the entry-size/num-entries header inline.
func anlzSectionAfterHeaderData(data []byte, tag string) []byte {
	blob := readANLZSection(data, tag)
	if blob == nil {
		return nil
	}
